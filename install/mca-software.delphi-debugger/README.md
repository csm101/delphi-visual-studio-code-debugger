...
This extension registers the `delphi` debug type and ships a Debug Adapter
...
The original debug type name, `delphi-win64`, is kept as an alias with the
same attributes, so an existing `launch.json` — hand-written or generated by
the RAD Studio plugin — needs no change. Both names debug both platforms.

**With [delphi-devkit (DDK)](https://marketplace.visualstudio.com/items?itemName=Snowcaloid.delphi-devkit)
installed, no `launch.json` is needed.** Right-click a project in DDK's tree and
choose **Debug** or **Attach Debugger**, or pick the project from the debug
dropdown: the executable (or the Host Application of a package), the symbols,
the sources, the packages and the run arguments come from DDK when the session
starts. A hand-written entry is one line — see
[Debugging a DDK project](#debugging-a-ddk-project).

> **Upgrading from the sideloaded version?** The old extension id was
> `local.delphi-win64-debug`. Both contribute the same debug types, so this
> version warns when the old copy is still installed and offers to remove it;
> accept, then reload the window.

**The same engine also talks to AI agents.** The download ships an **MCP
server** alongside this extension: 46 tools that let an agent set breakpoints,
...
### Debugging a DDK project

With [delphi-devkit](https://marketplace.visualstudio.com/items?itemName=Snowcaloid.delphi-devkit)
installed, a configuration names the project and nothing else:

```jsonc
{ "type": "delphi", "request": "launch", "name": "Debug MyApp", "ddkProject": "MyApp" }
```

`ddkProject` is a DDK project name, id, or the path of a `.dproj` / `.dpr` /
`.dpk`. When the session starts, DDK is asked for the project's debug target and
the configuration is completed from it: `program` (for a package or DLL
project, its Host Application), `mapFile` / `rsmFile`, `sourceRoot`,
`sourceSearchPaths`, `modules` (the project's own `.bpl` / `.dll` with their
symbols), `args` and `delphiProjectFile`. Anything you write yourself stays as
written. `"request": "attach"` attaches to the running executable, through the
process picker when several instances run. DDK's own warnings — a stale
`.rsm`, a package not built — are shown as notifications. You do not even have
to write that line: DDK's **Debug** / **Attach Debugger** context-menu entries
and its entries in the debug dropdown start exactly this configuration.

...
- [x] EXE MAP fallback -- a target built with a detailed MAP and NO embedded
      ...
      `MapOnlyGlobals`)
- [x] MAP of an image whose .text exceeds 4 MB: the segment-table parse stops
      at the end of the table, so detailed-map offsets past the preferred base
      no longer overwrite a segment base -- GitHub issue #12
      (`SegmentTable_PE32_IgnoresDetailedMapOffsetsAboveThePreferredBase`,
      `SegmentTable_PE32Plus_...`; synthetic MAP)
- [x] **Live, issue #12:** MAP-only target with 5.8 MB of .text on both
      bitnesses. A breakpoint in the program's main block (a second `.dpr`
      section, `.itext` on Win32) and one in a unit past 4 MB both bind, are
      HIT in order at their own file and line, and the caller frame resolves
      (`MapOnlyBigText_BreakpointsAreHitWithTheirSource`; fixture
      `MapOnlyBigText.dpr`, filler generated by nested includes). With the
      0.7.0 reader it fails as reported: Win64 "never stopped", Win32 "main-block
      breakpoint not verified"
- [x] MAP with several "Line numbers for" sections for one file (.text +
      .itext): every section is indexed, and survives the `.idx` sidecar
      (`LineNumbers_ItextSectionOfTheSameFile_IsIndexed`,
      `LineNumbers_ItextSectionOfTheSameFile_SurvivesTheSidecar`)
- [x] MAP address → line with interleaved sections: an address right after a
      generic instantiated inside a unit resolves to the unit's own file, and a
      record shared by two sections at a unit boundary goes to the later one

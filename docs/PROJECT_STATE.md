...
- **Assembly-level debugging — ALL SIX INCREMENTS DONE (2026-08-09).**
  ...
  instruction, with no line table required, surfaced on DAP
  (`granularity: "instruction"` on `next`/`stepIn`/`stepOut`) and MCP
  (`granularity` on `step_over`/`step_into`/`step_out`). Step-over runs past a
  `call` with a one-shot at PC + the DECODED length; a `rep`-prefixed
  instruction is completed the same way by BOTH into and over, because a
  trap-flag step at one retires a single ITERATION and leaves the PC where it
  was (measured, both bitnesses). Step-out takes the return address from
  `.pdata` on x64 / `[EBP+4]` on x86 and REFUSES where neither proves one.
  Everything refuses when the disassembler backend is unavailable — there is
  no fallback decoder in this project and a length is never guessed. DAP
  `readMemory`/`writeMemory` + `memoryReference`, MCP registers
  (`get_registers`/`set_register`), and a WOW64 register-write fix
  (`Wow64Get/SetThreadContext`, refusing `R8`..`R15` outright) round out the
  plan. The placeholder document a sourceless frame gets (increment 5, last)
  now carries real disassembly around the frame's PC instead of only a
  paragraph of explanation — measured in VS Code that the client opens the
  adapter's placeholder, never its own Disassembly View, so the placeholder
  has to carry that weight itself. Mechanism in `DAP_DEBUGGER_ARCHITECTURE.md`
  "Instruction granularity" and "The document's content — increment 5";
  decisions and their negative controls in `ASSEMBLY_LEVEL_DEBUGGING.md`;
  tests in `TEST_CATALOG.md` "O." through "T.". Fixed a latent HANG in the
  source-level step-over on the way: a transient step breakpoint rejected by
  the recursion guard was re-planted under the parked PC and re-trapped
  forever. Surfaced, not fixed, while building increment 5: an exception
  stop's reported frame 0 does not reliably resolve to the true faulting
  address in code with no debug info (`KNOWN_UNKNOWNS.md`).
...
- **A MAP's `SSSS:XXXXXXXX` is a linear address only in the segment table
  ...
  staying below the preferred base; on an image whose `.text` exceeds 4 MB the
  detailed map overwrote segment 1's base, and every MAP-bound breakpoint was
  reported verified but planted at an unmapped address. The scan now ends with
  the table. Verifying the fix on the real 139 MB / 359 MB Hydra2 MAPs
  (`DevTools\MapLineRvaProbe`) exposed three more MAP-reader defects that no
